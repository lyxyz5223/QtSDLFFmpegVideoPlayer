...
class AnimatedMenuStyle : public QProxyStyle
{
	

public:
	AnimatedMenuStyle(QStyle* style = 0) {}
...
void AnimatedMenuStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
...
void AnimatedMenuStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
	switch (element)
	{
	case QStyle::CE_MenuItem:
	{
		const AnimatedMenu* menu = dynamic_cast<const AnimatedMenu*>(widget);
		//内容绘制 区分类型
		if (const QStyleOptionMenuItem* mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option))
		{
			...
			else
			{
				...
				{
					//AnimatedMenuAction* action = menu->actionAt(menuItemRect.center());
					//if (!action) return;
...
int AnimatedMenuStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
	switch (metric)
	{
...
QSize AnimatedMenuStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
	switch (type)
	{
...
{
	AnimatedMenuStyle* s = new AnimatedMenuStyle();
	setStyle(s);
...
bool AnimatedMenu::eventFilter(QObject* watched, QEvent* e)
{
...
void AnimatedMenu::paintEvent(QPaintEvent* e)
{
	QPainter p(this);
...
void AnimatedMenu::mouseMoveEvent(QMouseEvent* e)
{
...
void AnimatedMenu::mouseReleaseEvent(QMouseEvent* e)
{
...
bool AnimatedMenu::event(QEvent* e)
{
...
void AnimatedMenu::showEvent(QShowEvent* e)
{
	startShowAnimation();
	QMenu::showEvent(e);
}

void AnimatedMenu::closeEvent(QCloseEvent* e)
{
	QMenu::closeEvent(e);
}
...
void AnimatedMenu::startShowAnimation()
{
	QParallelAnimationGroup* aniGroup = new QParallelAnimationGroup(this);
